...
const { data: homeData, status: dataStatus } = await useAsyncData(
  "home-data",
  async () => {
...
const recommendedCompetitions = computed(
  () => homeData.value?.competitions || []
);
  ...
  <div class="space-y-4 lg:space-y-8">
    ...
    >
      ...
      >
        ...
        <div
          class="absolute bottom-0 left-0 w-full p-4 md:p-6 bg-gradient-to-t from-black/80 to-transparent"
        >
          <h2 class="text-xl md:text-3xl font-bold text-white mb-2 truncate">
            {{ banner.title }}
          </h2>
      ...
      <div class="lg:col-span-8 space-y-4 lg:space-y-8">
        ...
        <section>
          <div class="flex items-center justify-between mb-4">
            <h3
              class="text-lg md:text-xl font-bold text-[var(--text-main)] flex items-center gap-2"
            >
              <Icon name="heroicons:fire" class="text-[var(--color-error)]" />
              热门赛事
            </h3>
            <n-button text type="primary" @click="navigateTo('/competitions')"
              >更多赛事 <Icon name="heroicons:arrow-right" class="ml-1"
            /></n-button>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
              ...
              <div class="p-4">
                <h4
                  class="font-bold text-lg text-[var(--text-main)] mb-2 truncate"
                >
                  {{ comp.title }}
                </h4>
                <div class="flex items-center gap-2 mb-3">
                  <n-tag
                    ...
                    size="small"
                    :bordered="false"
                    class="bg-gray-100 dark:bg-gray-700 text-[var(--text-secondary)]"
                  >
                    {{ tag }}
                  </n-tag>
                </div>
                ...
                >
                  <span class="flex items-center gap-1"
                    ...
                    {{ comp.participants }} 人参赛</span
                  >
                  <span
                    class="group-hover:text-[var(--color-primary)] transition-colors"
                    >查看详情</span
                  >
                </div>
              </div>
        ...
        <section>
          ...
          <div
            class="bg-[var(--bg-card)] rounded-xl border border-[var(--border-color)] overflow-hidden"
          >
            <n-list hoverable clickable>
              <n-list-item v-for="item in news" :key="item.id">
                <template #prefix>
                  <div
                    :class="`w-2 h-2 rounded-full ${item.type === 'warning' ? 'bg-[var(--color-warning)]' : item.type === 'success' ? 'bg-[var(--color-success)]' : 'bg-[var(--color-info)]'}`"
                  ></div>
                </template>
                <div class="flex items-center justify-between w-full gap-2">
                  <span
                    class="text-[var(--text-main)] font-medium truncate flex-1"
                    >{{ item.title }}</span
                  >
                  <span class="text-xs text-[var(--text-tertiary)] shrink-0">{{
                    item.date
                  }}</span>
                </div>
              </n-list-item>
      ...
      <div class="lg:col-span-4 space-y-4 lg:space-y-6">
        ...
        >
          ...
          <div class="grid grid-cols-2 gap-4 mb-6">
            <div
              class="bg-white/10 rounded-lg p-3 text-center backdrop-blur-sm"
            >
              <div class="text-2xl font-bold">
                {{ myStatus.activeCompetitions }}
              </div>
        ...
        <!-- 常用快捷入口 -->
        <div
          class="bg-[var(--bg-card)] rounded-xl border border-[var(--border-color)] p-5 shadow-sm"
        >
          <h4
            class="font-bold text-[var(--text-main)] mb-4 text-sm uppercase tracking-wider"
          >
            快捷入口
          </h4>
          <div class="grid grid-cols-2 gap-3">
            ...
            <n-button
              dashed
              class="h-16 flex flex-col gap-1 !rounded-lg opacity-60 hover:opacity-100"
            >
        ...
        <!-- 网站基本信息/公告栏 -->
        <div
          class="bg-[var(--color-primary)]/10 rounded-xl p-5 border border-[var(--color-primary)]/20"
        >
          <h4
            class="font-bold text-[var(--color-primary)] mb-2 flex items-center gap-2"
          >
            <Icon name="heroicons:information-circle" />
            关于平台
          </h4>
          <p class="text-sm text-[var(--text-secondary)] leading-relaxed">
            Yuna Nexus 是一个专注于 osu!
            社区赛事的综合管理平台。在这里，你可以轻松报名参赛、管理赛程、查看实时战况。
          </p>
