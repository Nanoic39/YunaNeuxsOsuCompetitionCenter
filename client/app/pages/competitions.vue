...
const { data: competitions } = await useAsyncData<Competition[]>('competitions-page', async () => {
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        <div class="h-48 relative overflow-hidden">
          <img :src="comp.image" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
          ...
          <div class="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 to-transparent p-4">
            <h3 class="text-white font-bold text-lg truncate">{{ comp.title }}</h3>
        ...
        <div class="p-4">
          <p class="text-[var(--text-secondary)] text-sm line-clamp-2 mb-4 h-10">
            {{ comp.description }}
          </p>
